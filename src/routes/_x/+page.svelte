...
<div class="w-full h-full flex flex-row justify-center items-center align-middle">
    <iframe src="" class="mx-auto w-[calc(100%-420px)] h-[calc(100%-32px)] bg-transparent" title="container"></iframe>
    <div class="flex flex-col ml-auto hover:w-[400px] hover:max-w-[400px] duration-500 max-w-[64px] w-[64px] overflow-clip sidepanel pt-32 h-full bg-crust rounded-l-2xl text-center">
        <div class="duration-300 w-full h-full delay-200 flex flex-col">
        <h2 class="gtitle font-bold text-3xl">_</h2>
        <gap class="flex flex-1"></gap>
        <div class="ring-2 ring-crust p-4 bg-mantle rounded-xl px-8 rounded-b-none">
            <button onclick="document.querySelector('iframe').requestFullscreen()" class="p-3 duration-300 backdrop-brightness-0 bg-mauve/30 text-mauve hover:bg-mauve/40 rounded-lg">
                <Maximize />
            </button>
        </div>
        </div>
