...
export default () => {
  ...
  setTimeout(async () => {
    ...
    for (let i = 0; i < assets.html.length; i++) {
      ...
      card.querySelector(".card-link").href =
        `/_x#${btoa(assets.html[i].name)}||${btoa("/assets/handlers/html.html#" + assets.html[i].id)}`;

      card.querySelector("img").src =
        `/assets/html/thumbs/${assets.html[i].id}.jpg`;
    ...
    for (let x = 0; x < assets.flash.length; x++) {
      ...
      card.querySelector(".card-link").href =
        `/_x#${btoa(assets.flash[x].name)}||${btoa("/assets/handlers/flash.html#" + assets.flash[x].id)}`;
      card.querySelector("img").src =
        `/assets/flash/thumbs/${assets.flash[x].id.split(".")[0]}_flash.jpg`;
      document.querySelector(".gcontainer").appendChild(card);
      card.id = "";
    }
    ...
    for (let y = 0; y < idx.length; y++) {
      ...
      for (let b = 0; b < assets.rom[idx[y]].length; b++) {
        ...
        card.querySelector("img").src =
          `/assets/rom/thumbs/${assets.rom[idx[y]][b].id.split(".")[0]}_${idx[y]}.jpg`;
        card.querySelector(".card-link").href =
          `/_x#${btoa(assets.rom[idx[y]][b].name)}||${btoa(`/assets/handlers/rom.html#${idx[y]};${assets.rom[idx[y]][b].id}`)}`;
        card.querySelector("img").addEventListener("error", () => {
          card.querySelector("img").src = "/assets/missing.png";
        });
        document.querySelector(".gcontainer").appendChild(card);
    ...
    for (let c = 0; c < pidx.length; c++) {
      let templatecard = document.querySelector("#template");
      let card = templatecard.cloneNode(true);
      card.querySelector(".title").innerText = toUpper(
        pidx[c].replaceAll("-", " "),
      );
